...
public interface FindMapper {
	...
	@Select("select user_email userEmail, user_id userId from users where user_name=#{userName} && user_email=#{userEmail} ")
	public User getUserId(FindInfo fInfo);
	
	@Select("select user_email userEmail, user_pw userPw from users where user_id=#{userId} && user_email=#{userEmail} ")
